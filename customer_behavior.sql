...
select item_purchased ,ROUND(avg(review_rating),2) as average_review_rate from customer  
...
select shipping_type , round(avg(purchase_amount),2) from customer
 where shipping_type in ('Standard','Express')
  ...
  from customer group by subscription_status order by total_customer,avg_spend desc;
